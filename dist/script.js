    ...
    elem.forEach(function (item, index) {
      item.addEventListener(event, function () {
        switch (item.nodeName) {
          // проверка ноды, с которой взаимодействует пользователь
          case 'SPAN':
            state[prop] = index;
            break;
          ...
          case 'INPUT':
            if (item.getAttribute('type') === 'checkbox') {
              ...
              elem.forEach(function (box, i) {
                // функционал выбора только одного чекбокса
                box.checked = false;
...
var forms = function forms(state) {
  ...
  formsCollection.forEach(function (item) {
    item.addEventListener('submit', function (event) {
      ...
      if (item.getAttribute('data-calc') === 'end') {
        // если конечная страница формы - добавит данные из калькулятора
      ...
      postData('assets/server.php', formData).then(function (result) {
        ...
        setTimeout(function () {
          ...
          windowsCollection.forEach(function (item) {
...
var showGallery = function showGallery() {
  ...
  gallerySection.addEventListener('click', function (event) {
    ...
    if (target && target.classList.contains('preview')) {
      ...
      var path = target.parentNode.getAttribute('href'); // ссылка на изображение большого размера из контейнера над img

      bigImage.setAttribute('src', path);
    }

    if (target && target.matches('div.popup')) {
      // отслеживается клик на подложку
      imgPopup.style.display = 'none';
    }
  });
};
